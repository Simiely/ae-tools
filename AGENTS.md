...
> 📌 **文档基线**：2026-09-21（commit `待本轮 docs commit 回填`）**全仓技术债清零**：给 5 个零测试面板（AE-Water-Rise-Generator 1.1.2 / AE-Lyrics-Animator 3.7 / AudioScale 1.0.1 / starry-sky-generator 3.4 / AE-Dashed-Grid-Generator 1.0.2）补断言测试，共 **+238 条**，此前 4,656 行代码零回归保护。两种取函数方式：**导出闸门**（首选，位置须在纯函数之后、任何 AE API 之前）与**文本提取**（外壳是裸块时用，块内 `return` 不合法）。每个面板都做了反向对照（注入 16 处回归 → 25 条断言变红）。验收 11/11 + 骨架 45/45，技术债 6 项全清零
