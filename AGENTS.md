...
> 📌 **文档基线**：2026-09-21（commit `待本轮 docs commit 回填`）**MountainSpectrum v1.5.7：「山峰 柱」可设父级跟随运动而不毁掉山形** ——根因是**坐标系混用**：形状基准原先取自柱图层自身的 `transform.position`，设父级后 AE 会补偿该值 → 与峰点（合成坐标）不再同系 → 形散（实测 33/40 根变化）；改挂到**「山峰 控制器」**（空对象、不跟随）上，于是**山形只由控制点决定**。**基准层（控制器 / 总高度闸 / 山峰点）不可设父级**，诊断里会报父级状态。断言 273 → 285，反向对照 3 处注入 → 10 条变红。（本轮同时完成：5 个零测试面板补 238 条断言 → 技术债 6 项全清零）
