...
## v0.3.12（2026-09-21）—— 面板顶部显示版本号

- 新增 `VER` 常量（版本号单一真相：文件头注释 / 常量 / CHANGELOG 顶部三处一致）
- 面板顶部常驻显示 `QuickKey 快速K帧  v0.3.12`
- 为什么必须：ScriptUI Panel 由 **AE 启动时**载入 —— 改完脚本不重启 AE，面板里跑的还是旧版，
  AE 报的错也可能是旧文件的行号。没有版本号就无法判断 AE 里跑的是不是最新的。
- **顺带修复**：文件头版本号此前停在 `0.3.6`，而 CHANGELOG 已到 `0.3.11` —— 现以 `VER` 常量为单一真相。

## v0.3.11(2026-08-19)— 修复「使用存储/复位/导入配置」后头部与曲线开关控件不刷新

- **真机问题**:「使用存储」恢复槽位后,「数值输入」下拉(1 空/2 空/3 空/表达式)停在旧值
- **根因**:载入槽位走「改 state → refresh()」单向流,但 `refreshHeader` 只刷新了
  表达式行显隐和按钮文案,**没有把 state 同步回控件**;`refreshCurve` 也没同步曲线开关
- **修复**:`refreshHeader` 现在同步 4 个控件(模式下拉 / 节点数输入框 / 数值类型下拉 /
  表达式框);`refreshCurve` 同步 2 个控件(曲线总开关 / 端点平滑);相关控件提升为
  块级变量(cntInp/ddMode/ddType/exprInp/chkCurve/chkSmooth),构建时赋值
- 影响范围:「使用存储 1-4」「复位」「导入配置」三类操作(统一走 refresh 的都会被修复)
- 纯 UI 层改动,158 断言不变;语法 ✓ BOM ✓ 已部署,待真机确认

## v0.3.10(2026-08-19)— 内置曲线预设回退:去掉 5 个 cubic 英文,回到默认 4 个中文

- 用户确认不需要 v0.3.6 新增的 cubic 系列,内置预设 9 → **4 个**(线性/缓入/缓出/缓入缓出)
- 只改 `PRESETS_DEFAULT`(唯一数据源,下拉/导出/导入/分组自动跟随);已导入的自定义预设不受影响
- 测试 158 断言不变;语法 ✓ BOM ✓ 已部署

## v0.3.9(2026-08-19)— 导出文件可读性优化:单行条目 + 内置/导入分组 + 备注说明

- 用户反馈:导出 JSON 每条预设多行嵌套,读起来累(且会手动编辑后重新导入)
- **导出预设**(stringifyPresets):每条预设**单行**(`{ "name": "线性", "x1": 0, ... }`);
  **内置 9 个一组、导入的在后一组,组间空行**(段行);顶层新增 `_comment` 字段
  说明文件用途与字段含义(备注)
- **导出配置**(stringifyConfig):参数**一行一个**、曲线库同预设格式(单行+分组)、
  槽位单行;同样加 `_comment` 备注(含全部参数含义说明)
- **保持标准 JSON**:仍是合法 JSON(node/JSON.parse 可直接读、任何工具可格式化),
  解析端(validatePresets / parseConfigText / 手写提取)忽略 `_comment`;
  用户编辑文件后「导入」路径完全不变(JSON 分支优先)
- 纯序列化层改动;测试 155 → **158 断言**(_comment 存在 + 分组空行);语法 ✓ BOM ✓ 已部署
