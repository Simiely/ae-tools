...
> 📌 **文档基线**：2026-09-21 · 山峰频谱 MountainSpectrum **v1.5.6** / 仓库 **v1.3.23**（v1.5.6：修「起伏曲线」枚举越界分叉 —— 审计用 1000 次逐根比对发现纯函数与表达式对非法 `ftype` 结论不同，两侧统一「Math.round 后夹到 [0,3]」；新增第十二节边界等价性断言（25 用例 × 40 根）已做反向对照。272 断言全绿。）
