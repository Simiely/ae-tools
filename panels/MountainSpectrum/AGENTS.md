...
> 📌 **文档基线**：2026-09-21 · 山峰频谱 MountainSpectrum **v1.5.7** / 仓库 **v1.3.27**（v1.5.7：**「山峰 柱」可设父级跟随运动而不毁掉山形** —— 根因是形状基准原先取自柱图层自身的 `transform.position`，被父级带走后与峰点坐标不再同系；改为挂在**「山峰 控制器」**（空对象、不跟随）上，于是**山形只由控制点决定**。断言 273 → 285，新增第十三节「柱图层设父级 / 拖动不再影响形状」，已做反向对照（3 处注入 → 10 条变红）。⚠️ 基准层（控制器 / 总高度闸 / 山峰点）不可设父级，诊断里会报父级状态。）
