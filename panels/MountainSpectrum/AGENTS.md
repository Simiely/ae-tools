...
> 📌 **文档基线**：2026-09-18 · 山峰频谱 MountainSpectrum **v1.0.0** / 仓库 **v1.3.18**
> （审计后重构：删除「影响范围」「影响范围倍率%」「边缘控制」三个失效/隐藏参数；宽度只由左右边缘点控制；
> 新增参数表 PARAM_SPECS 与 8 个"改完立即生效"参数；修负基础高度分叉、魔数漂移、静默失败。
> 192 条断言含 1600 次逐根比对全过，关键体检断言做过反向对照。commit hash 见根 AGENTS.md）
