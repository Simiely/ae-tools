...
//
// v1.5.7 修「柱图层设父级后山形消失」(用户需求:「希望控制点的位置不变, 也能有效果」):
...
(function (thisObj) {
    ...
    function buildExprCore(i, N, M) {
        ...
        L.push("var bx = c.transform.position[0] + (i - (N - 1) / 2) * pitch;");
        ...
        L.push("if (HL) {");
        L.push("  hGate = 1;");
        // v1.5.7: 基线 Y 同样改用控制器位置(理由见 bx 处) —— 总闸不设父级, 两者同坐标系
        L.push("  hMax = c.transform.position[1] - HL.transform.position[1];");
        ...
        L.push("      pk2 = c.transform.position[1] - P2.transform.position[1];");
        ...
        L.push("    pk = c.transform.position[1] - pp[1];");
    ...
    //
    // v1.5.7 起基准 bx / 基线 Y 取自【控制器】而非本图层 → 本图层可放心设父级跟随运动,
    //   形状计算不受父级影响; 同时"拖动本图层 = 纯净平移整行", 不再连带改形(详见 buildExprCore)。
    ...
    var TIP_TEXT =
        ...
        + "【行位置】拖「山峰 柱」图层 = 平移整行(形状不变 —— 山形只由控制点决定)。\n"
        + "【跟随运动 / 父级】可给「山峰 柱」设父级让它跟随运动, 山形不受影响。\n"
        + "    ⚠️ 但「山峰 控制器」「山峰 总高度闸」「山峰点」【不要】设父级 —— 它们是山形的基准,\\n"
        + "       设了父级形状会跟着漂。山形基准挂在「山峰 控制器」上, 别删它、也别移动它(除非想改山形)。\n"
    ...
    function dumpDiagnostics() {
        ...
        if (!comp) {
            ...
            diag("图层: 柱=" + (bars ? "✓" : "✗") + " 控制=" + (ctrl ? "✓" : "✗")
                + " 高度总闸=" + (hgt ? "✓" : "✗"));
            // v1.5.7: 父级状态 —— 山形基准挂在「控制器」上, 若基准层被设了父级, 形状会漂移。
