...
//
// v1.5.8 修正 v1.5.7 的方向错误(用户澄清:「山峰柱根据它的位置跟控制点互动, 看是否起伏」):
//   - v1.5.7 把形状基准挂到「控制器」上 → 结果是"柱子带着山形整体走", 山形【永远不变】。
//     但用户要的不是这个: 控制点是【固定在合成里的地形】, 柱是【移动的采样者】——
//     柱走到哪儿, 就按哪儿的地形高度长。柱被父级带走后离控制点远了, 就【该】不起伏。
//   - 正确做法: 基准改用【本图层在合成空间的真实位置】= thisLayer.toComp(transform.anchorPoint)。
//     · 无父级时它【恒等于】transform.position → 与最初行为完全一致(零变化);
//     · 设父级瞬间 AE 会补偿 position 但视觉位置不变 → toComp 不变 → 形状不变(符合直觉);
//     · 父级一动, 真实位置变化 → 高度按新位置重算 → "柱子扫过固定地形" ✓ 这才是要的效果。
//   - 对比 v1.5.7 的错法: 拿【父级坐标系】的 transform.position 直接减峰点坐标(合成坐标) → 差一个父级偏移。

// v1.5.7 (方向有误, 见上) 曾把基准挂到「控制器」上(用户需求:「希望控制点的位置不变, 也能有效果」):
//   - 现象: 给「山峰 柱」设父级(想让整座山跟随运动)后, 山峰形状整个散掉。
//   - 根因【坐标系混用, 不是父级功能的问题】: 表达式的高度计算要拿「整行基准 X/Y」与「峰点坐标」相减,
//     而基准原先取自【本图层自身的 transform.position】—— 它一旦被设了父级就变成【父级坐标系】的值,
//     峰点没父级(仍是合成坐标) → 两者差一个父级偏移 → 距离算出负数/错值 → 形状散掉。
//     实测(40 根取样): 只有柱的坐标系平移 → 33/40 根高度改变; 全部一起平移 → 0/40 改变。
//   - 修法: 基准改用【「山峰 控制器」的位置】(c.transform.position) —— 它是空对象、不该跟随运动,
//     于是基准与峰点始终同处合成坐标系 → 形状恒定; 而柱图层可自由设父级跟随 → 视觉上整座山跟随运动。
//   - 等价性: 生成时控制器与「山峰 柱」同置 [centerX, baseY](见 generate), 故本改动对现有用法无行为变化。
//   - 副作用(属修正): 拖动「山峰 柱」图层现在只平移整行、不再改变山形 —— 这正是本文件一贯的设计意图
//     (「拖动形状图层移动整行」), 此前因基准取自图层自身而实际会连带变形。
...
(function (thisObj) {
    ...
    function buildExprCore(i, N, M) {
        ...
        // v1.5.7: 整行基准 X 从【本图层位置】改为【控制器位置】。
        //   为什么必须换: bx 要与峰点坐标(合成坐标)相减。若 bx 取自本图层 transform.position,
        //   一旦给它设父级, 该值就变成【父级坐标系】的值, 而峰点没父级 → 差一个父级偏移 → 山形散掉。
        //   控制器是空对象、不跟随运动 → 基准与峰点恒在同一坐标系 → 形状恒定, 而本图层可自由设父级。
        //   等价性: generate() 里控制器与本图层同置 [centerX, baseY], 故初始行为完全一致。
        // v1.5.8: 先求【本图层在合成空间的真实位置】—— 山形要按"柱子实际在哪儿"采样控制点地形。
        //   ⚠️ 不能用 transform.position: 设父级后它是【父级坐标系】的值, 与峰点(合成坐标)不同系。
        //   toComp(anchorPoint) 恒为合成坐标 —— 无父级时等价于 transform.position(行为不变),
        //   有父级时随父级运动而变 → "柱子扫过固定不变的地形, 到哪儿按哪儿起伏"。
        L.push("var wp = thisLayer.toComp(transform.anchorPoint);");
        L.push("var bx = wp[0] + (i - (N - 1) / 2) * pitch;");
        ...
        L.push("if (HL) {");
        L.push("  hGate = 1;");
        L.push("  hMax = wp[1] - HL.transform.position[1];");
        ...
        L.push("      pk2 = wp[1] - P2.transform.position[1];");
    ...
    //    所以只有输出与图层位置无关, 拖动形状图层才能移动整行
    //    (v0.1.1 的 bug: bx 取自控制器坐标又被减掉图层坐标, 结果图层拖不动)。
    //    柱子水平偏移只由 W/G 决定 → 图层可自由拖动, 行整体跟随。
    //
    // v1.5.8 起基准 bx / 基线 Y 取自【本图层的合成空间真实位置】(toComp) ——
    //   给本图层设父级后, 它跟随运动时会"扫过"固定不变的控制点地形, 高度按新位置重算;
    //   没有父级时 toComp ≡ transform.position, 行为与最初完全一致。
    ...
    var TIP_TEXT =
        ...
        + "【行位置】拖「山峰 柱」图层 = 移动整行 —— 高度按新位置对控制点地形重新采样。\n"
        + "【跟随运动 / 父级】v1.5.8: 给「山峰 柱」设父级后, 柱跟随运动时会【扫过】固定不变的控制点地形 ——\n"
        + "    走到哪儿就按哪儿的地形起伏, 离开控制点影响范围就回基础高度。\n"
        + "    ⚠️ 但「山峰 控制器」「山峰 总高度闸」「山峰点」【不要】设父级 —— 它们是地形本身,\n"
        + "       设了父级地形会跟着动, 起伏就乱了。也别删它们(删峰点 = 地形塌了)。\n"
    ...
    function dumpDiagnostics() {
        ...
        if (!comp) {
            ...
            diag("图层: 柱=" + (bars ? "✓" : "✗") + " 控制=" + (ctrl ? "✓" : "✗")
                + " 高度总闸=" + (hgt ? "✓" : "✗"));
            // v1.5.8: 父级状态 —— 地形层(峰点/缘点/总闸/控制器)被设父级 = 地形跟着动, 起伏会乱;
            //   「山峰 柱」则【可以】设父级跟随运动(高度按真实位置重新采样地形)。
            diag("父级: 柱=" + (bars && bars.parent ? bars.parent.name : "无")
                + " | 控制器=" + (ctrl && ctrl.parent ? (ctrl.parent.name + " <-- !! 基准层不该有父级") : "无(正确)")
                + " | 总闸=" + (hgt && hgt.parent ? (hgt.parent.name + " <-- !! 基准层不该有父级") : "无(正确)"));
